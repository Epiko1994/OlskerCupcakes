    ...
    public static ArrayList<User> customerReader() throws SQLException, ClassNotFoundException {
        ...
        for (User user : userList) {
            ...
            for (Order order : user.getOrders()) {
                preparedStatement = connection.prepareStatement("SELECT order_id, amount, base_name, topping_name, base_price + topping_price as 'price' from order_lines inner join base b on order_lines.base_id = b.base_id inner join topping t on order_lines.topping_id = t.topping_id where order_id = ?");
